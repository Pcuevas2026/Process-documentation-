...
/***************************************--GO CLIENTE LUNES--***************************************/
DELETE from EXTRACCIONES.dbo.mxrisk_EVER_SEMANAL WHERE SEMANA_Cosecha >= 202423;

truncate table POLITICAS2020.dbo.empresario;
truncate table POLITICAS2020.dbo.COLOCACION_EMPRESARIO;
truncate table POLITICAS2020.[dbo].[GT_OTROS_PEDIDOS_EMPRESARIO];
truncate table POLITICAS2020.[dbo].NuevaColocacionEmpresario;

truncate table EXTRACCIONES.dbo.ETL_ClienteLunes;
truncate table EXTRACCIONES.dbo.PROC_MXSolicitudeshist_actor;
truncate table EXTRACCIONES.dbo.ETL_PEDIDO_SIN_SALDO_FROM_MX;

truncate table SILO.dbo.Cli_photo_ClientesSinSaldo;
truncate table  EXTRACCIONES.DBO.ETL_CAPACIDAD_PAGO_X_SEMANA;

--truncate table CAMPANIAS.dbo.ClienteAbandono4Gestion;

truncate table JOB_RISK.dbo.ETL_HIST_COSECHAS;

truncate table EXTRACCIONES.dbo.CU_FROM_MX
/***************************************--SEGUIMIENTO CLIENTE SEMANAL--***************************************/
-- use SILO;

--select A.* from [10.53.63.61,1534].[BDREPDEVLAM].dbo.envioGT A
--WHERE NOT EXISTS(
--	select * from silo.dbo.CLI_SEGUIMIENTOSEMANAL_FUJOAGENCIA B
--	WHERE a.corte = b.corte
--);

select top 10 * from SILO.dbo.CLI_SEGUIMIENTOSEMANAL_FUJOAGENCIA; -- LISTO EN DW
/***************************************--EVER--***************************************/
-- use BDREPDEVLAM;

--select * from [BDREPDEVLAM].DBO.GT_ever_semanal;

select top 10 * from EXTRACCIONES.dbo.mxrisk_EVER_SEMANAL -- LISTO EN DW
/***************************************--EMPRESARIO--***************************************/
--use BDREPDEVLAM;

--select * from BDREPDEVLAM.dbo.empresarioGT;

--select top 10 * from POLITICAS2020.dbo.empresario; -- LISTO EN DW

--select * from BDREPDEVLAM.dbo.pedidos292CU;

--select top 10 * from POLITICAS2020.dbo.COLOCACION_EMPRESARIO; -- LISTO EN DW

--select * from BDREPDEVLAM.dbo.GT_OTROS_PEDIDOS_EMPRESARIO;

--select top 10 * from POLITICAS2020.dbo.GT_OTROS_PEDIDOS_EMPRESARIO; -- LISTO EN DW

--select * from BDREPDEVLAM.dbo.GT_EmpresarioSendGT;

--select top 10 * from POLITICAS2020.dbo.NuevaColocacionEmpresario; -- LISTO EN DW
/***************************************--CLIENTE--***************************************/
-- use BDREPDEVLAM;

--select * from BDREPDEVLAM.dbo.SendtoGT_ClienteUnico; -- Vienen registros con FechaPrimerCompra >= 2021

select top 10 * from EXTRACCIONES.dbo.ETL_ClienteLunes; -- Registros con FechaPrimerCompra >= 2021
/***************************************--PASO A NORMAL--***************************************/
--use EXTRACCIONES;

DELETE cli
from EXTRACCIONES.dbo.GT_cliente_Lunes cli
JOIN EXTRACCIONES.dbo.ETL_ClienteLunes etl_cli
	ON etl_cli.Finotienda = cli.Finotienda
	and etl_cli.Ficteid = cli.Ficteid
	and etl_cli.Fidigitover = cli.Fidigitover

--INSERT INTO GT_cliente_Lunes
--select * from ETL_ClienteLunes;

select top 10 * from EXTRACCIONES.dbo.GT_cliente_Lunes; -- LISTO EN DW
/***************************************--ACTOR JVC MX--***************************************/
-- use BDREPDEVLAM;

--select * from BDREPDEVLAM.dbo.SEndClienteGT;
select top 10 * from EXTRACCIONES.dbo.PROC_MXSolicitudeshist_actor; -- LISTO EN DW
/***************************************--CARTERA JV GT--***************************************/
--use COVID;
--truncate table Cartera_actual_JVC;

--INSERT INTO Cartera_actual_JVC
--select
--	DISTINCT
--	l.Pais,
--	l.Departamento,
...
--	CAST(ISNULL(t.fiFolio,0) AS VARCHAR(8)) AS CU,
--	t.FICANAL,
--	t.FISUCURSAL,
--	t.FINOPEDIDO,
...
--	geo.fnLongitud
--from
--	ALMACENRISK.dbo.TemporalDiario T
--LEFT JOIN(
--	select
--		z.*,
...
--	from(
--		select *, ROW_NUMBER() OVER(PARTITION BY ClienteUnico ORDER BY fdfecsolicitud DESC) AS cca
...
--LEFT JOIN EXTRACCIONES.dbo.PROC_GEOCLIENTE geo
--	ON t.fiCanalcu = geo.fiCanal
--	AND t.FISUCURSALCU = geo.fisucursal
--	AND t.FIFOLIO = geo.fiFolio
--WHERE t.FIPERATRAACUM <= 39;

select top 10 * from COVID.dbo.Cartera_actual_JVC; -- LISTO EN DW
/***************************************--PEDIDOS SIN SALDO--***************************************/
--use BDREPDEVLAM;

--select * from BDREPDEVLAM.dbo.GT_envioGUATEPedidosCero;

select top 10 * from EXTRACCIONES.dbo.ETL_PEDIDO_SIN_SALDO_FROM_MX;
/***************************************--CAPACIDADES--***************************************/
--use BDREPDEVLAM;

--select * from BDREPDEVLAM.dbo.DROP_ENVIOGUATE2;

select top 10 * from SILO.dbo.MI_CapacidadSemanal; -- LISTO EN DW
/***************************************--INSERTAR ETL PEDIDOS POR SKU--***************************************/
--use BDREPDEVLAM;

--select * from BDREPDEVLAM.DBO.GUA_GT_PASO_RESERVA_INGRESO_COSECHAS;

select top 10 * from JOB_RISK.dbo.ETL_HIST_COSECHAS
/***************************************--PEDIDOS POR SKU--***************************************/
-- use JOB_RISK;

--INSERT INTO JOB_RISK.DBO.HIST_COSECHASGT 
--select * from job_risk.dbo.ETL_HIST_COSECHAS A
--WHERE NOT EXISTS(
--	select
--		FICANAL,
--		FISUCURSAL,
--		FINOPEDIDO
--	from
--		JOB_RISK.DBO.HIST_COSECHASGT B
--	WHERE
--		B.FICANAL = A.FICANAL
...
select top 10 * from JOB_RISK.DBO.HIST_COSECHASGT; -- LISTO EN DW
/***************************************--NUEVAS CAPACIDADES--***************************************/
--use BDREPDEVLAM;

--select * from BDREPDEVLAM.dbo.DROP_ENVIOGUATE3 with(nolock)

select top 10 * from EXTRACCIONES.dbo.ETL_CAPACIDAD_PAGO_X_SEMANA;
/***************************************--NUEVAS CAPACIDADES--***************************************/
--use EXTRACCIONES;

--INSERT INTO EXTRACCIONES.dbo.PROC_CAPACIDAD_PAGO_X_SEMANA
--select *,convert(date,GETDATE()) FechaInsert
--from EXTRACCIONES.dbo.ETL_CAPACIDAD_PAGO_X_SEMANA A
--where not exists(
--	select 1 from EXTRACCIONES.dbo.PROC_CAPACIDAD_PAGO_X_SEMANA B
...
select top 10 * from EXTRACCIONES.dbo.PROC_CAPACIDAD_PAGO_X_SEMANA;
/****************************************************************************************************/